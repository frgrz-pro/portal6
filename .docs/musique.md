...
| AzuraCast | Phases 1–2 validées le 2026-08-20, **re-vérifiées le 2026-09-06** (méthode officielle inchangée, parc Docker sain, ports 80/443/2022/8000 libres, `/mnt/m` monté). ✅ **DÉPLOYÉ le 2026-09-06** (Phase 3 franchie) : conteneurs `azuracast` + `azuracast_updater`, canal stable, interface joignable sur `http://localhost` et `http://192.168.0.5`. Reste : créer le super-admin, puis **Phase 4** (brancher `M:\music` sans duplication). Côté consommation : l'app [ha-remote](app-remote.md) a un onglet Radio (2026-09-07) qui lira les stations dès qu'il y en aura une. Détail : [hardware/design-serveur-azuracast.md](hardware/design-serveur-azuracast.md) |
